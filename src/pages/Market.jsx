  ...
  useEffect(() => {
    const fetchMachines = async () => {
      ...
      try {
        ...
        // ✅ FIX: supports BOTH array and {machines: []}
      ...
      {machines.map((machine) => (
        <div className="machine-card" key={machine._id}>
          ...
          <div className="machine-header">
            <img
              src={
                machine.img?.startsWith("http")
                  ? machine.img
                  : `${process.env.REACT_APP_API_URL}/${machine.img}`
              }
              alt={machine.name}
              onError={(e) => {
                e.target.src = "/fallback.png";
          ...
          <div className="machine-actions">
            ...
            <button
              className={`buy ${
                balance < machine.price || buyingId === machine._id
                  ? "disabled"
                  : ""
              }`}
              disabled={
      ...
      {showDetails && selectedMachine && (
        <div className="modal-overlay">
          <div className="details-modal">
            ...
            <img
              src={
                selectedMachine.img?.startsWith("http")
                  ? selectedMachine.img
                  : `${process.env.REACT_APP_API_URL}/${selectedMachine.img}`
              }
              alt={selectedMachine.name}
              onError={(e) => {
                e.target.src = "/fallback.png";
