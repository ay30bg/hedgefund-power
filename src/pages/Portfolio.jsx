...
/* ---------------- Investment Helpers ---------------- */
const getInvestmentStatus = (start, days) => {
  const now = new Date();
  const startDate = new Date(start);
...
const getInvestmentProgress = (start, days) => {
  const now = new Date();
  const startDate = new Date(start);
  const endDate = new Date(startDate.getTime() + days * 86400000);
...
export default function Portfolio() {
  ...
  // ⏱ Refresh UI every second (for progress bars)
  useEffect(() => {
    const interval = setInterval(() => forceUpdate((n) => n + 1), 1000);
    return () => clearInterval(interval);
  }, []);
      ...
      {activeTab === "investments" && (
        <section className="machines-section">
          <div className="machine-cards">
            ...
            {loadingInvestments ? (
              [...Array(4)].map((_, i) => (
                <div className="machine-card skeleton-card" key={i}></div>
              ))
            ) : investments.length === 0 ? (
              <p>No investments yet</p>
            ) : (
              investments.map((inv) => {
                const status = getInvestmentStatus(inv.startDate, inv.days);
                const progress = getInvestmentProgress(inv.startDate, inv.days);
